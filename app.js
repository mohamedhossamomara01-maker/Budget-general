...
function calcCarryover(mk, monthly, entries, indExtra, deletedXl) {
  ...
  const prevInc = pn("salary") + pn("transport") + pn("waste") + pn("old") + pn("deals") + pn("eid") + pn("dohaa") + pn("magdy") + ((prevSaved.incomeExtra||[]).reduce((s,x)=>s+(+x.amount||0),0));
  const prevFix = pn("car_fixed") + pn("rent") + pn("internet") + pn("charity") + pn("mom") + pn("ajz") + pn("tahwish") + ((prevSaved.fixedExtra||[]).reduce((s,x)=>s+(+x.amount||0),0));
...
function SalaryModal({
  ...
  const [f, sf] = useState({
    ...
    ajz: g("ajz") || 0,
    tahwish: g("tahwish") || 0,
    incomeExtra: g("incomeExtra") || [],
    fixedExtra: g("fixedExtra") || []
  });
  const [newIncLbl, setNewIncLbl] = useState("");
  const [newIncAmt, setNewIncAmt] = useState("");
  const [newFixLbl, setNewFixLbl] = useState("");
  const [newFixAmt, setNewFixAmt] = useState("");
  const [newFixLoan, setNewFixLoan] = useState("");
  const [y, m] = mk.split("-").map(Number);
  const n = k => +(f[k] || 0);
  const sumItems = arr => (arr || []).reduce((s, x) => s + (+x.amount || 0), 0);
  const incomeExtraSum = sumItems(f.incomeExtra);
  const fixedExtraSum = sumItems(f.fixedExtra);
  const inc = n("salary") + n("transport") + n("waste") + n("old") + n("deals") + n("eid") + n("dohaa") + n("magdy") + incomeExtraSum;
  const fix = n("car_fixed") + n("rent") + n("internet") + n("charity") + n("mom") + n("ajz") + n("tahwish") + fixedExtraSum;
  const rem = inc - fix - n("home_given");
  const addIncomeItem = () => {
    const amt = parseFloat(newIncAmt);
    if (!newIncLbl.trim() || !amt) return;
    sf(p => ({ ...p, incomeExtra: [...(p.incomeExtra || []), { id: Date.now(), label: newIncLbl.trim(), amount: amt }] }));
    setNewIncLbl(""); setNewIncAmt("");
  };
  const delIncomeItem = id => sf(p => ({ ...p, incomeExtra: (p.incomeExtra || []).filter(x => x.id !== id) }));
  const addFixedItem = () => {
    const amt = parseFloat(newFixAmt);
    if (!newFixLbl.trim() || !amt) return;
    sf(p => ({ ...p, fixedExtra: [...(p.fixedExtra || []), { id: Date.now(), label: newFixLbl.trim(), amount: amt, loanId: newFixLoan || null }] }));
    setNewFixLbl(""); setNewFixAmt(""); setNewFixLoan("");
  };
  const delFixedItem = id => sf(p => ({ ...p, fixedExtra: (p.fixedExtra || []).filter(x => x.id !== id) }));
  const Fld = ({
    k,
    lbl,
    note
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: { fontSize: 10, color: "#4a6080", marginTop: -2, marginBottom: 9 }
  }, "أي مصدر دخل تاني (بدل، عيدية، فلوس قديمة، أي حاجة) — ضيفه هنا:"),
  (f.incomeExtra || []).map(it => /*#__PURE__*/React.createElement("div", {
    key: it.id,
    style: { display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0f1a2a", border: `1px solid ${T.bdr}`, borderRadius: 10, padding: "8px 10px", marginBottom: 6 }
  },
    /*#__PURE__*/React.createElement("span", { style: { fontSize: 12, color: "#e2e8f0" } }, it.label),
    /*#__PURE__*/React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
      /*#__PURE__*/React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: T.green } }, fmt(it.amount), " ج"),
      /*#__PURE__*/React.createElement("span", {
        onClick: () => delIncomeItem(it.id),
        style: { color: T.red, cursor: "pointer", fontSize: 13 }
      }, "✕")
    )
  )),
  /*#__PURE__*/React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 9 } },
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 0, flex: 1.3 },
      type: "text", placeholder: "اسم المصدر", value: newIncLbl,
      onChange: e => setNewIncLbl(e.target.value)
    }),
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 0, flex: 1 },
      type: "number", inputMode: "decimal", placeholder: "المبلغ", value: newIncAmt,
    ...
    style: { fontSize: 10, color: "#4a6080", marginTop: -2, marginBottom: 9 }
  }, "أي مصروف ثابت شهري (إيجار، نت، قسط...) — ضيفه هنا، ولو بيتخصم من قرض اربطه بيه"),
  (f.fixedExtra || []).map(it => /*#__PURE__*/React.createElement("div", {
    key: it.id,
    style: { display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0f1a2a", border: `1px solid ${T.bdr}`, borderRadius: 10, padding: "8px 10px", marginBottom: 6 }
  },
    /*#__PURE__*/React.createElement("div", null,
      /*#__PURE__*/React.createElement("span", { style: { fontSize: 12, color: "#e2e8f0" } }, it.label),
      it.loanId && loans && loans.find(l => l.id === it.loanId) && /*#__PURE__*/React.createElement("span", { style: { fontSize: 9, color: T.purple, marginRight: 6 } }, "🔗 ", loans.find(l => l.id === it.loanId).name)
    ),
    /*#__PURE__*/React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
      /*#__PURE__*/React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: T.red } }, fmt(it.amount), " ج"),
      /*#__PURE__*/React.createElement("span", {
        onClick: () => delFixedItem(it.id),
        style: { color: T.red, cursor: "pointer", fontSize: 13 }
      }, "✕")
    )
  )),
  /*#__PURE__*/React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" } },
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 0, flex: 1.3 },
      type: "text", placeholder: "اسم المصروف", value: newFixLbl,
      onChange: e => setNewFixLbl(e.target.value)
    }),
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 0, flex: 1 },
      type: "number", inputMode: "decimal", placeholder: "المبلغ", value: newFixAmt,
      onChange: e => setNewFixAmt(e.target.value)
    }),
    /*#__PURE__*/React.createElement("button", {
  ...
  loans && loans.length > 0 && /*#__PURE__*/React.createElement("select", {
    value: newFixLoan,
    onChange: e => setNewFixLoan(e.target.value),
    style: { ...S.inp, marginBottom: 9, fontFamily: "'Cairo',sans-serif" }
  },
    /*#__PURE__*/React.createElement("option", { value: "" }, "بدون ربط بقرض/قسط"),
    loans.map(l => /*#__PURE__*/React.createElement("option", { key: l.id, value: l.id }, "🔗 اربط بـ: ", l.name))
  ), /*#__PURE__*/React.createElement("div", {
    style: {
      ...S.row,
      borderTop: `1px solid ${T.bdr}`,
...
function SummaryScreen({
  ...
  const incomeExtraSum = (saved.incomeExtra || []).reduce((s, x) => s + (+x.amount || 0), 0);
  const fixedExtraSum = (saved.fixedExtra || []).reduce((s, x) => s + (+x.amount || 0), 0);
  const baseInc = n("salary") + n("transport") + n("waste") + (n("old") > 0 ? n("old") : autoOld) + n("deals") + n("eid") + n("dohaa") + n("magdy") + incomeExtraSum;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    val: n("magdy"),
    c: T.green
  }), (saved.incomeExtra || []).map(it => /*#__PURE__*/React.createElement(R, {
    key: it.id,
    icon: "💵",
    lbl: it.label,
    val: it.amount,
    ...
    style: S.card()
  }, n("car_fixed") > 0 && /*#__PURE__*/React.createElement(R, {
    icon: "🚗",
    lbl: "قسط العربية",
    val: n("car_fixed"),
    c: T.red
  }), n("rent") > 0 && /*#__PURE__*/React.createElement(R, {
    icon: "🏠",
    lbl: "قسط الشقة / الإيجار",
    val: n("rent"),
    c: T.red
  }), n("internet") > 0 && /*#__PURE__*/React.createElement(R, {
    icon: "📡",
    lbl: "الإنترنت",
    val: n("internet"),
    c: T.red
  }), n("charity") > 0 && /*#__PURE__*/React.createElement(R, {
    icon: "🤲",
    lbl: "الصدقات والحصري",
    val: n("charity"),
    c: T.red
  }), n("mom") > 0 && /*#__PURE__*/React.createElement(R, {
    icon: "👩",
    lbl: "أمي",
    val: n("mom"),
    c: T.red
  }), (saved.fixedExtra || []).map(it => /*#__PURE__*/React.createElement(R, {
    key: it.id,
    icon: "🔒",
    lbl: it.label,
    val: it.amount,
  ...
  }, fmt(fixDisplay || 0), " ج"))), /*#__PURE__*/React.createElement("div", {
    style: S.sub
  ...
  }, [["الثوابت", fixDisplay || 0, T.red], ["الأكل والبيت", mHomeTot, T.blue], ["صيانة", mCar, "#a78bfa"], ["💰 تحويش", SUM((entries||[]).filter(e=>e.type==="home"&&e.cat==="saving"&&finKey(e.date)===mk)), "#a78bfa"], ["⛽ بنزين", ind ? (ind.petrol||0) : 0, "#f59e0b"], ["🧾 ضريبة", ind ? (ind.tax||0) : 0, "#f59e0b"], ["🔧 نفخ كاوتش", ind ? (ind.tire||0) : 0, "#f59e0b"]].map(([l, v, c]) => /*#__PURE__*/React.createElement("div", {
    key: l,
    style: {
      textAlign: "center"
    }
...
function GoalsScreen({
  ...
  const [newLoanOriginal, setNewLoanOriginal] = useState("");
  const [newLoanQist, setNewLoanQist] = useState("");
  const addLoan = () => {
    const orig = parseFloat(newLoanOriginal);
    if (!newLoanName.trim() || !orig) return;
    onSaveLoans([...loansList, { id: "loan_" + Date.now(), name: newLoanName.trim(), original: orig, qist: newLoanQist.trim() }]);
    setNewLoanName(""); setNewLoanOriginal(""); setNewLoanQist("");
  };
  const delLoan = id => onSaveLoans(loansList.filter(l => l.id !== id));
  const loanRemaining = (loan) => {
    let rem = +loan.original || 0;
    Object.keys(monthly || {}).forEach(mk => {
      if (mk === "_loans") return;
      const items = (monthly[mk] && monthly[mk].fixedExtra) || [];
      items.forEach(it => { if (it.loanId === loan.id) rem -= (+it.amount || 0); });
    });
  ...
  const dp = ch.length ? Math.round(ch.filter(c => c.done).length / ch.length * 100) : 0;
  function LoanCard({
    icon,
    lbl,
    color,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: S.sub
  }, "💳 القروض والأقساط (محدّث تلقائياً)"),
  loansList.map((loan, i) => /*#__PURE__*/React.createElement("div", { key: loan.id, style: { position: "relative" } },
    /*#__PURE__*/React.createElement(LoanCard, {
      icon: "💳",
      lbl: loan.name,
      color: [T.purple, T.blue, T.orange, T.green][i % 4],
      original: loan.original,
      remaining: loanRemaining(loan),
      qist: loan.qist ? loan.qist + " ج" : "—",
      note: "اربط أي مصروف ثابت بيه من شاشة تعديل بيانات الشهر"
    }),
    /*#__PURE__*/React.createElement("span", {
      onClick: () => delLoan(loan.id),
      style: { position: "absolute", top: 10, left: 10, color: T.red, cursor: "pointer", fontSize: 14 }
    }, "✕")
  )),
  /*#__PURE__*/React.createElement("div", { style: { ...S.card(), marginBottom: 10 } },
    /*#__PURE__*/React.createElement("div", { style: { fontSize: 12, color: "#8fa3c4", marginBottom: 8 } }, "➕ ضيف قرض / قسط (سلفة، عربية، شقة...)"),
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 6 },
      type: "text", placeholder: "اسم القرض/القسط", value: newLoanName,
      onChange: e => setNewLoanName(e.target.value)
    }),
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 6 },
      type: "number", inputMode: "decimal", placeholder: "الأصل (إجمالي المبلغ)", value: newLoanOriginal,
      onChange: e => setNewLoanOriginal(e.target.value)
    }),
    /*#__PURE__*/React.createElement("input", {
      style: { ...S.inp, marginBottom: 8 },
      type: "text", placeholder: "قيمة القسط الشهري (اختياري، للعرض بس)", value: newLoanQist,
      onChange: e => setNewLoanQist(e.target.value)
    }),
    /*#__PURE__*/React.createElement("button", {
      onClick: addLoan,
      style: { ...S.btn(T.blue) }
...
// DUHA GOALS SCREEN
// ══════════════════════════════════════════════════════════════
const DUHA_CHECK_DEF_DEFAULT = [];
const DUHA_GOALS_DEF_DEFAULT = [];
...
function App({ profile, setProfile }) {
  ...
  const saveLoans = useCallback((loansArr) => {
    setMo(p => ({ ...p, _loans: loansArr }));
  }, []);
