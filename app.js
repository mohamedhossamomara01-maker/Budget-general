...
function App() {
  ...
  const [cats, setCats] = useState(() => ld(`cats_${ld("budget_uid","")}`, CATS));
  const [incomes, setIncomes] = useState(() => ld(`inc_${ld("budget_uid","")}`, []));
  const [installments, setInstallments] = useState(() => ld(`inst_${ld("budget_uid","")}`, []));
