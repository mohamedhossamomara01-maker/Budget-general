...
function SetupScreen({ onDone }) {
  ...
  return React.createElement("div", { style:{ minHeight:"100vh", background:T.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 } },
    ...
    React.createElement("div", { style:{ fontSize:22, fontWeight:900, color:T.text, marginBottom:6, textAlign:"center" } }, "أهلاً بيك!"),
    React.createElement("div", { style:{ fontSize:13, color:T.sub, marginBottom:28, textAlign:"center" } }, "ابتدي تنظم ميزانيتك دلوقتي"),
    React.createElement("div", { style:{ width:"100%", maxWidth:400 } },
      ...
      React.createElement("div", { style:{ fontSize:11, color:T.sub, marginBottom:4, textAlign:"right" } }, "مرتبك (ج)"),
      ...
      React.createElement("button", { style:S.btn(), onClick:submit }, "ابتدي →")
    )
  );
}
...
// SUMMARY SCREEN
// ══════════════════════════════════════════════════════════════
function SummaryScreen({ uid, profile, entries, mk, onMkChange }) {
  const salary = profile.salary || 0;
  const month = entries.filter(e => finKey(e.date) === mk);
  const mTot = SUM(month);
  const mSaving = SUM(month.filter(e=>e.cat==="saving"));
  const balance = salary - mTot;
  ...
  return React.createElement("div", null,
    ...
    React.createElement("div", { style:{ ...S.card(balance>=0?"#10b98122":"#ef444422"), border:`2px solid ${balance>=0?T.green:T.red}`, textAlign:"center", marginBottom:10 } },
      ...
      React.createElement("div", { style:{ fontSize:11, color:T.sub, marginTop:4 } }, fmt(salary)," دخل — ",fmt(mTot)," مصاريف")
    ),
...
// SETTINGS SCREEN
// ══════════════════════════════════════════════════════════════
function SettingsScreen({ uid, profile, onUpdate }) {
  ...
  return React.createElement("div", null,
    ...
    React.createElement("div", { style:S.sub }, "الأقساط الثابتة"),
    React.createElement("div", { style:{ ...S.card(), color:T.sub, fontSize:12, textAlign:"center" } }, "قريباً — هتضيف أقساطك وديونك هنا")
  );
}
...
function App() {
  ...
  return React.createElement("div", { style:{ background:T.bg, minHeight:"100vh", color:T.text, fontFamily:"system-ui,sans-serif", direction:"rtl", maxWidth:480, margin:"0 auto", paddingBottom:70 } },
    ...
    React.createElement("div", { style:{ padding:"0 12px" } },
      tab === "summary" && React.createElement(SummaryScreen, { uid, profile, entries, mk, onMkChange:setMk }),
      tab === "expenses" && React.createElement(ExpensesScreen, { uid, userName:profile.name, entries, onAdd:addE, onDel:delE, mk }),
      tab === "goals" && React.createElement(GoalsScreen, { uid }),
      tab === "settings" && React.createElement(SettingsScreen, { uid, profile, onUpdate:setProfile })
    ),
